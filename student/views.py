...
def student_dashboard_view(request):
    ...
    announcements = QMODEL.Announcement.objects.all()[:3]
    return render(
        request,
...
def student_grades_view(request):
    ...
    results = list(
        ...
        .order_by('exam__course_name', '-date')
    )
    for r in results:
        r.total_possible_marks = _result_total_possible_marks(r)
        if r.bundle:
            r.exam_type_display = r.bundle.get_exam_type_display()
        else:
            r.exam_type_display = '—'
    ...
    for r in results:
        ...
        if key not in course_groups:
            course_groups[key] = {'course': r.exam, 'entries': []}
        course_groups[key]['entries'].append(r)

    return render(request, 'student/student_grades.html', {
        'course_groups': list(course_groups.values()),
    })
